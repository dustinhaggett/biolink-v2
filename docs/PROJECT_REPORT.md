...
BioLink v2 builds directly on the open-source v1 codebase released alongside the prior research paper [4]. The v1 system was an academic prototype: a Jupyter notebook orchestrating training, evaluation, and inference, with a minimal Streamlit demo intended for showing reviewers how the model behaved on a few example queries. Most v2 development was therefore not about training a new model but about building the production infrastructure around the trained weights — the calibration pipeline, the entity resolution layer, the parallel enrichment, the evidence-grounded UI, and the deployment configuration. The v1 model weights are preserved unchanged in the v2 repository and remain the source of all predictions; the v2 contribution is the application layer that makes those predictions accessible and useful.
...
The CTD data required substantial preprocessing before it could be used for training. The raw chemical-disease association table contains over 1.6 million entries spanning multiple evidence types — curated, inferred via gene overlap, inferred via phenotype overlap — of which only the curated and well-supported inferred associations are appropriate as positive labels for our task. We filtered the corpus to retain associations with direct evidence types and deduplicated compound synonyms; CTD lists, for example, several variants of the same compound under slightly different canonical names, but our model should treat them as the same drug. After preprocessing, approximately 380,000 positive drug-disease pairs remained.

We constructed negative examples by random pairing of drugs and diseases that did not co-occur in the curated CTD data. The positive-to-negative ratio was set to 1:1, since the model architecture is simple enough that more aggressive negative sampling did not improve held-out performance during exploratory experiments. The final dataset was partitioned into training (80%), validation (10%), and test (10%) splits with a fixed random seed. We considered entity-level splits — ensuring that no drug or disease appearing in test ever appeared in training — but rejected this for two reasons. First, every drug and disease in CTD has co-occurrences with multiple partners, so an entity-level split would severely reduce the effective dataset size. Second, the deployed model is intended to predict associations between known entities rather than generalize to novel ones, so a random-pair split is the relevant evaluation regime.

Hyperparameter selection used a small grid search over hidden-layer width (128, 256, 512), dropout rate (0.2, 0.3, 0.5), and learning rate (1e-3, 5e-4, 1e-4). The selected configuration — 256 hidden units, 0.3 dropout, learning rate 5e-4 with the Adam optimizer — achieved the best validation AUC. Training ran for 20 epochs with early stopping on validation loss; the best epoch was typically reached between epochs 8 and 12. We did not observe overfitting in the conventional sense: training loss continued to drop slowly even after validation loss plateaued, suggesting that the model's effective capacity was well-matched to the dataset size. We were initially concerned that random negative sampling would produce unrealistically easy examples, but in practice the model's strong AUC on a held-out test set (0.947) suggests that the random pairings were difficult enough to force the model to learn meaningful associations rather than memorizing surface features.

Evaluation beyond AUC included calibration error (Expected Calibration Error of 0.024 after temperature scaling, down from 0.087 before), top-K accuracy on a curated set of known repurposing candidates (the top twenty predictions for diseases such as multiple myeloma included thalidomide, lenalidomide, and bortezomib — all established treatments), and qualitative review by team members familiar with the biomedical literature. The qualitative review proved the most informative: AUC is a useful summary statistic but does not by itself reveal whether the model is making predictions a human expert would consider plausible. The model also has known weaknesses on certain query types. Queries for very broad diseases ("cancer," "infection") return generic results because the training data treats specific subtypes as the canonical entities. Drugs that share names across multiple chemical classes can produce confused predictions, a known issue with CTD synonymy. Diseases that have been added to CTD only recently may be poorly represented in the BioWordVec embedding space, since the embeddings were trained on a fixed snapshot of PubMed.

The choice of an MLP over more elaborate architectures — graph neural networks, transformer-based knowledge graph embeddings — was deliberate. The MLP is small enough to run inference in milliseconds on CPU, which is essential for a consumer-facing deployment that runs on Hugging Face Spaces' free tier without GPU acceleration.
...
Robust error handling proved more important than we initially anticipated. Each external API has its own failure modes: PubMed E-utilities occasionally rate-limits requests despite our use of the recommended throttle; OpenFDA returns 404s for drugs whose canonical name diverges from the FDA's preferred labeling; ClinicalTrials.gov sporadically returns malformed JSON during peak traffic; and Perplexity sometimes returns extra prose alongside the requested JSON object, requiring lenient parsing. We wrap each API call in a try/except block that catches both network errors and parsing failures, logs the specific failure for later analysis, and substitutes a sentinel "unavailable" value into the result. A partial enrichment is always better than a query failure. We also added lightweight observability: each query generates a structured log entry recording the resolved CTD entity, the model's top scores, the latency of each enrichment call, and a count of any API failures encountered. These logs are inspected manually during development; for production deployment they would be routed to a structured log aggregator, but that was out of scope for the project.
...
The choice to bucket calibrated probabilities into three discrete tiers — rather than presenting raw percentages — was informed by usability testing. Early testers consistently misinterpreted small differences in raw probability (say, 78% versus 82%) as clinically meaningful when in fact the calibration uncertainty makes such differences essentially noise. The three-tier presentation forces the user to think about the prediction at the right level of resolution: a "Strong" candidate warrants serious investigation, a "Moderate" candidate warrants caution, and a "Speculative" candidate should be treated as a lead requiring substantial further evidence.

The PDF export uses a structured single-page-per-candidate layout containing the drug name, calibrated probability, verdict, evidence summary, mechanism pathway, evidence quality, interaction warnings, and a list of supporting trials with NCT identifiers. The CSV export carries the same fields in a flat tabular format suitable for downstream analysis. Both exports include a footer disclaimer reiterating that BioLink is a research tool and not medical advice.
...
Deployment uses a Dockerfile that installs Python 3.11, the project dependencies, and the cached BioWordVec embeddings — precomputed and stored as compact numpy arrays to avoid the multi-gigabyte runtime load of the full embedding model. Hugging Face Spaces builds the Docker image on each push to the connected git repository and serves the application at a public URL. The cached-embedding optimization was essential: loading the full 13 GB BioWordVec file at startup would have exceeded the free-tier memory limit, while the precomputed per-entity embeddings consume well under one hundred megabytes and load in seconds.

A concrete example illustrates the workflow. During development of the Perplexity integration, we encountered intermittent crashes when parsing the model's JSON response. The crashes were not reproducible — sometimes the same query would succeed three times in a row and fail on the fourth. Manual debugging suggested that the JSON parser was choking on trailing characters in the response, but we could not identify any pattern. We pasted the failing inputs into Claude Code along with the stack trace and the relevant parsing code, and within two iterations Claude Code identified that Perplexity occasionally returned a JSON object followed by an English-language postscript ("Note: the above evidence is preliminary...") that our parser was not stripping. The fix was a five-line change to extract the first valid JSON object via brace-counting rather than naive end-of-string parsing. This kind of debugging — where the bug is rare, the failure mode is opaque, and the fix is small once identified — is where AI development tools provide the most leverage.

Team coordination relied on git for code, GitHub Issues for tracking, and weekly synchronous reviews for design decisions. We partitioned responsibilities along the natural layer boundaries: one team member focused primarily on the model and calibration layers, one on the enrichment integrations, and one on the UI and export pipeline. This partitioning worked well because the layer interfaces were stable from early in the project, and most of the integration friction came at the boundaries — particularly between the async enrichment runner and the UI rendering — which we addressed in joint debugging sessions.
...
Several limitations warrant explicit acknowledgment. The training data captures only documented drug-disease associations, so truly novel repurposing candidates may score low simply because the relevant biological connection has not yet been studied in the published literature. The evidence verdict system depends on Perplexity's literature retrieval, which has its own coverage gaps and biases — preprints and non-English literature are under-represented, and the structured prompt we use occasionally elicits oversimplified verdicts. Calibration was performed on CTD-derived data, so the calibrated probabilities are most reliable for the kinds of drug-disease pairs CTD documents thoroughly; predictions for under-studied conditions should be interpreted with extra caution.

Several directions for future work are evident. A graph neural network trained on the CTD knowledge graph directly, rather than on flattened embedding pairs, might capture higher-order relationships that the current MLP cannot. Periodic retraining as CTD updates would keep the model current without requiring user intervention. Integration with electronic health record data would enable personalized recommendations that account for individual patient context, though this would require substantially more rigorous validation and institutional oversight before any clinical use. We hope BioLink demonstrates that responsibly-deployed AI in a sensitive domain need not require a heroic engineering effort, and that small teams can build genuinely useful clinical-adjacent tools when modern AI tooling is brought to bear thoughtfully.
