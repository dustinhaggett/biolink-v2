...
We relied heavily on AI-assisted development tools throughout the project. Claude Code (via Anthropic's CLI) was the primary tool, applied to architecture iteration, API integration scaffolding, debugging across the asyncio pipeline, and substantial portions of the UI components; GitHub Copilot handled line-by-line completion during early development. The biggest gains came from repetitive integration work and multi-file refactors, while architectural decisions and pipeline-level debugging still required significant human oversight.

Deployment is via a Dockerfile on Hugging Face Spaces. We pre-compute and ship per-entity BioWordVec embeddings as compact numpy arrays so the app can run without loading the full 13 GB BioWordVec file at startup, which would have exceeded the free tier's memory limit. We wrote unit tests for the core inference, calibration, entity resolution, and explanation modules, with API integrations tested against mocked responses; manual testing against the deployed application caught most integration issues, and usability testing with non-expert users informed late-stage UI changes.
...
The MLP assigns confidence based on knowledge-graph patterns, which don't always reflect clinical reality. An early test query returned cyclosporine — an immunosuppressant — at 99% confidence for Lyme disease, a bacterial infection where immunosuppression would be actively harmful. The model was picking up cyclosporine's frequent co-occurrence with inflammatory conditions, not a genuine therapeutic relationship. We addressed this by grounding every prediction in published literature via Perplexity Sonar and surfacing the model's confidence and the evidence verdict as two independent signals; when the model says "99% Strong" and the evidence verdict says "Evidence Conflicts," the disagreement itself is informative.

### B. API Cost Management

Querying Perplexity on all twenty candidates per request would have cost roughly ten cents per query — fine for a demo but prohibitive for a self-funded student project. We adopted a tiered approach: Perplexity runs only on the top five candidates where it has the most information value; the free APIs (PubMed, OpenFDA, ClinicalTrials.gov) handle baseline enrichment for all twenty results; and we use Claude Haiku rather than the more expensive Anthropic models for entity resolution and explanation, where the accuracy difference for these constrained tasks is minimal. The end result is two to three cents per query, which is sustainable.

### C. Entity Resolution for Free-Text Input

CTD identifies diseases by canonical names like "Myocardial Infarction," but real users type "heart attack" or "MI." The model only produces meaningful scores when the input is mapped to a recognized CTD entity. We implemented a two-tier resolution: Claude Haiku handles the primary mapping with a system prompt that returns both a canonical name and a confidence level, falling back to `difflib` fuzzy matching when the API is unavailable. When Haiku reports low confidence, the UI surfaces a clarification dialog. One subtle bug we hit: Haiku sometimes returned "low" confidence even for exact matches, triggering unnecessary clarifications; we fixed the intent mapper to skip clarification when the returned entity is already a valid CTD identifier.

### D. Reverse Search Architecture

The model was originally designed only for disease-to-drug search, but a common user need is the inverse: someone taking a drug wants to know what other conditions it might treat. We pre-compute and cache all 2,525 disease embeddings at model load time (1.9 MB extra memory), enabling instant reverse queries. The new `score_all_diseases()` method mirrors `score_all_drugs()` exactly because the MLP's feature vector is symmetric — concatenating embeddings then computing their absolute difference and element-wise product produces meaningful representations regardless of which entity is held fixed. A subtle gotcha: a transposed argument order would have produced wrong scores rather than an outright crash, so we preserved ordering carefully.

### E. PDF Export with Unicode

The most stubborn issue arose in PDF export. The fpdf2 library defaults to fonts that support only latin-1, and Perplexity's evidence text routinely contains em dashes, smart quotes, and other Unicode characters that crash the generator. Simply stripping non-latin-1 characters was not enough — the fpdf2 cursor drifts after automatic page breaks, producing "not enough horizontal space" errors on certain code paths. We solved this with two helper functions: a `_safe()` preprocessor that maps common Unicode characters to ASCII equivalents (em dash to hyphen, smart quote to straight quote) and drops anything remaining outside latin-1, plus a `_mc()` wrapper for `multi_cell` calls that explicitly resets the cursor to the left margin before each emission.

## V. Conclusion

BioLink shows that a lightweight MLP trained on BioWordVec embeddings, paired with modern LLM tooling and evidence-grounding APIs, can turn a research-grade drug-disease classifier into something genuinely useful to patients, clinicians, and researchers. The biggest takeaways from building it were that calibration, evidence grounding, and deployment constraints matter as much as raw model accuracy when moving from a notebook to a user-facing system. The transformer baseline confirmed that domain-specific BioWordVec embeddings provide a meaningful lift on this task (AUC 0.947 vs 0.912), and post-hoc temperature scaling brought the expected calibration error from 0.087 down to 0.024 with a single learned parameter.

Future work could explore graph neural networks trained directly on the CTD knowledge graph rather than on flattened embedding pairs, periodic retraining as CTD updates, and — with appropriate clinical validation and institutional oversight — integration with electronic health record data for context-aware recommendations.
